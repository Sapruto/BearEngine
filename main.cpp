...
// НЕ НАДО ЭТО: #define GLAD_GL_IMPLEMENTATION - это только в одном .c файле!

// ========== 1. СНАЧАЛА ОПРЕДЕЛЕНИЯ ==========
#define IMGUI_IMPL_OPENGL_LOADER_GLAD
#define GLFW_INCLUDE_NONE  // Важно! Не даем GLFW включать OpenGL

// ========== 2. ПОТОМ OpenGL (GLAD) ==========
#include <glad/glad.h>  // ИСПРАВЛЕНО: было glad/glad.h, а надо просто glad/glad.h? 

// ========== 3. ПОТОМ GLFW ==========
#include <GLFW/glfw3.h>

// ========== 4. ПОТОМ СТАНДАРТНЫЕ БИБЛИОТЕКИ ==========
#include <ctime>
#include <chrono>
#include <thread>
#include <iostream>
...
struct GameState {
    ...
    static ResourceManager* resourceManager;  // Добавляем указатель на ResourceManager
    static SpriteRenderer* spriteRenderer;    // Добавляем указатель на рендерер
};
...
ResourceManager* GameState::resourceManager = nullptr;
SpriteRenderer* GameState::spriteRenderer = nullptr;
...
// Функция для создания спрайта с правильными параметрами
std::unique_ptr<Sprite> CreateSprite(const std::string& texturePath, float sizeX, float sizeY) {
    if (!GameState::resourceManager || !GameState::spriteRenderer) {
        std::cerr << "Error: ResourceManager or SpriteRenderer not set!" << std::endl;
        return nullptr;
    }
    
    auto sprite = std::make_unique<Sprite>(*GameState::resourceManager, *GameState::spriteRenderer, texturePath);
    ...
    void Update() override {
        ...
        if (enemyTrans && playerTrans && shootTimer >= shootCooldown) {
            ...
            if (distance < shootRange) {
                ...
                // Создаем спрайт с помощью вспомогательной функции
                auto bulletSprite = CreateSprite("Assets/textures/enemy_bullet.png", 10.0f, 10.0f);
                if (bulletSprite) {
...
public:
    ...
    void Update() override {
        ...
        if (shoot && timeSinceLastShot >= shootCooldown) {
            ...
            // Создаем спрайт с помощью вспомогательной функции
            auto bulletSprite = CreateSprite("Assets/textures/bullet_player.png", 10.0f, 10.0f);
            if (bulletSprite) {
                bullet->AddComponent(bulletSprite.release());
            }
...
private:
    ...
    void SpawnEnemy(const char* texturePath, Vector2 scale, float mass = 1.0f, float drag = 0.5f) {
        ...
        // Создаем спрайт с помощью вспомогательной функции
        auto sprite = CreateSprite(texturePath, 40.0f, 40.0f);
        if (sprite) {
...
int main() {
    ...
    // 9. Создаем ResourceManager
    std::cout << "\nSTEP 9: Creating ResourceManager..." << std::endl;
    ResourceManager resourceManager;
    std::cout << "ResourceManager created" << std::endl;
    
    // Сохраняем указатели в глобальном состоянии
    GameState::resourceManager = &resourceManager;
    GameState::spriteRenderer = spriteRenderer;
    
    std::cout << "\nSTEP 10: Loading resources..." << std::endl;
    resourceManager.LoadResource("Assets/textures/player.png", ResourceType::Texture);
    resourceManager.LoadResource("Assets/textures/enemy_normal.jpg", ResourceType::Texture);
    resourceManager.LoadResource("Assets/textures/enemy_fast.png", ResourceType::Texture);
    resourceManager.LoadResource("Assets/textures/enemy_tank.png", ResourceType::Texture);
    resourceManager.LoadResource("Assets/textures/enemy_shooter.png", ResourceType::Texture);
    resourceManager.LoadResource("Assets/textures/enemy_bomber.png", ResourceType::Texture);
    resourceManager.LoadResource("Assets/textures/enemy_healer.png", ResourceType::Texture);
    resourceManager.LoadResource("Assets/textures/bullet_player.png", ResourceType::Texture);
    resourceManager.LoadResource("Assets/textures/enemy_bullet.png", ResourceType::Texture);
    std::cout << "All textures loaded" << std::endl;
    
    std::cout << "\nSTEP 11: Creating scene..." << std::endl;
    ...
    std::cout << "\nSTEP 12: Creating game objects..." << std::endl;
    ...
    // Создаем спрайт игрока с помощью вспомогательной функции
    auto playerSprite = CreateSprite("Assets/textures/player.png", 30.0f, 30.0f);
    if (playerSprite) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        // Создаем спрайт врага с помощью вспомогательной функции
        auto enemySprite = CreateSprite("Assets/textures/enemy_normal.png", 40.0f, 40.0f);
        if (enemySprite) {
    ...
    std::cout << "\nSTEP 13: Entering main loop..." << std::endl;
    ...
    while (!drawing->ShouldClose()) {
        ...
        // Рендеринг (сбор данных для отрисовки)
        graphics->Update();
        
        // Обновление логики (здесь могут удаляться объекты)
        scene->UpdateScene();
    ...
    // Очищаем глобальные указатели
    GameState::resourceManager = nullptr;
    GameState::spriteRenderer = nullptr;
    
    // TagManager все еще синглтон, его нужно очистить
