...
void Font::BuildAtlas(){
    ...
    for (int i = 32; i <= 126; i++){
        ...
        if (glyphWidth == 0 || glyphHeight == 0) {
            ...
            data.bearing = Vector2f(0, 0);
            data.size = Vector2f(data.advance, fontSize);
        ...
        data.advance = slot->advance.x >> 6;
        data.bearing = Vector2f(slot->bitmap_left, slot->bitmap_top);
        data.size = Vector2f(glyphWidth, glyphHeight);
