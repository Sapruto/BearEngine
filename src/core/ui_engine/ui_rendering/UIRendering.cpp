...
#include "ShaderPathsUI.h"

UIRendering::UIRendering() 
    : shader(ShaderPathsUI::Base + "/UIVertexShader.glsl", 
             ShaderPathsUI::Base + "/UIFragmentShader.glsl") {
    CreateBuffers();
...
void UIRendering::RenderCanvas(Canvas* canvas) {
    ...
    Matrix4x4f projection = Matrix4x4f::ortho(0.0f, screenWidth, screenHeight, 0.0f, -1.0f, 1.0f);
    glUniformMatrix4fv(uniforms.projection, 1, GL_FALSE, projection.GetData());
