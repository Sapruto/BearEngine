...
Vector2 MousePosToCanvasPos(const Vector2& mousePos, Canvas* canvas){
    return mousePos; // пока так, а потом когда у Canvas может быть много состояний надо сделать типа преобразование нормальное
}

bool Button::CheckMousePos(const Vector2& globalMousePos, Canvas* canvas){
    Vector2 realPos = MousePosToCanvasPos(globalMousePos, canvas);
...
void Button::Update(){
    ...
    Vector2 globalMousePos = Vector2(InputSystem::GetInstance().GetMouseX(), InputSystem::GetInstance().GetMouseY());
