...
HierarchyViewer::HierarchyViewer(Editor* editor, HierarchyViewerUI* ui) 
    : editor(editor), ui(ui) {}
...
void HierarchyViewer::Start() {
    ...
    for (auto* gameObject : currentGameScene->GetGameObjects()) {
        GameObject* uiObject = ui->CreateUIObject(gameObject, index, editor->GetInspectorLogic());
